...
//		Log.debug("  [Note: In bmp format this is row " + bmpRow + "]");
	...
	public void readScene(byte[] fileBytes) throws FileNotFoundException {
		...
		// read view
		Point eye = readPoint(scanner); //Punto de vista
		Point center = readPoint(scanner); //Centro
		Vector up = readVector(scanner); //Vector
		double fovy = scanner.nextDouble(); //Follaje ???? ke
		camera = new Camera(eye, center, up, fovy, cols, rows); //Crear una camara desde el punto de vista
